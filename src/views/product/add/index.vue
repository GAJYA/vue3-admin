<template>
  <el-card class="box-card">
    <template #header>
      ...
      <span class="add-product">添加商品</span>
    </template>
    <el-tabs
      v-model="activeName"
    >
...
import type { IAttrList, ICategoryTree, IRuleDetail, IHeader } from '@/api/types/product'
...
const productCates = ref<ICategoryTree[]>([]) // 商品分类
const product = ref({
  activity: computed(() => activities.value.map(item => item.name)),
  attrs: [] as IAttrList[], // 商品规格
  cate_id: [] as number[],
  command_word: '',
  couponName: [],
  coupon_ids: [],
...
// const multiAttrData = ref<IAttrList[]>([])
